...
@section('style')
    <style>
        .flash-row { animation: flashBg .5s; }
        @keyframes flashBg { 0% { background: #d4edda; } 100% { background: transparent; } }
        .table-danger td { background-color: #f8d7da !important; }

        /* IMPORT ZONE */
        .import-zone {
            border: 2px dashed #c0c9d5; border-radius: 10px; padding: 28px 24px;
            background: #f8fafc; transition: border-color .2s, background .2s;
        }
        .import-zone:hover { border-color: #4e73df; background: #eef2ff; }
        .import-zone .import-icon { font-size: 2.5rem; color: #4e73df; }

        /* INFO PANEL */
        #import-info-panel { display: none; }
        #import-info-panel.has-content { display: block; }
        .info-item {
            display: flex; align-items: flex-start; gap: 8px;
            padding: 7px 10px; border-radius: 6px; font-size: .84rem;
            margin-bottom: 5px;
        }
        .info-item.info-warn  { background: #fff8e1; border-left: 3px solid #ffc107; }
        .info-item.info-error { background: #fdecea; border-left: 3px solid #dc3545; }
        .info-item.info-ok    { background: #e8f5e9; border-left: 3px solid #28a745; }
        .info-item i { margin-top: 1px; flex-shrink: 0; }

        /* REPEATER CARD */
        .repeater-wrapper { display: flex; flex-direction: column; gap: 20px; }
        .resi-card {
            border: 1px solid #dee2e6; border-radius: 10px; overflow: hidden;
            box-shadow: 0 2px 8px rgba(0,0,0,.06); transition: box-shadow .2s;
        }
        .resi-card:hover { box-shadow: 0 4px 18px rgba(0,0,0,.11); }
        .resi-card-header {
            background: #2f3237; color: #fff; padding: 10px 18px;
            display: flex; align-items: center; justify-content: space-between;
            font-weight: 600; font-size: .95rem; letter-spacing: .02em;
        }
        .resi-card-header .badge-number {
            background: rgba(255,255,255,.25); border-radius: 20px;
            padding: 2px 12px; font-size: .85rem; margin-right: 8px;
        }
        .resi-card-body { padding: 20px; background: #fff; }
        .btn-remove-resi {
            background: rgba(255,255,255,.18); border: 1px solid rgba(255,255,255,.4);
            color: #fff; border-radius: 6px; padding: 3px 10px;
            font-size: .82rem; cursor: pointer; transition: background .15s;
        }
        .btn-remove-resi:hover { background: rgba(220,53,69,.7); border-color: transparent; }
        .btn-add-resi {
            border: 2px dashed #4e73df; color: #4e73df; background: transparent;
            border-radius: 8px; padding: 12px; font-weight: 600;
            transition: background .15s, color .15s; width: 100%;
        }
        .btn-add-resi:hover { background: #4e73df; color: #fff; }
        .resi-card .table th { font-size: .82rem; white-space: nowrap; }
        .resi-card .table td { vertical-align: middle; font-size: .85rem; }
        .section-label {
        ...
        /* FILE RESI PREVIEW */
        .file-resi-wrap { position: relative; }
        .file-resi-preview {
            display: none; margin-top: 8px;
            border: 1px solid #dee2e6; border-radius: 8px; overflow: hidden;
            max-height: 220px; cursor: pointer; position: relative;
        }
        .file-resi-preview img {
            width: 100%; object-fit: contain; max-height: 220px;
        ...
        .file-resi-preview:hover .preview-overlay { opacity: 1; }
        ...
        .file-resi-badge {
            ...
            font-size: .78rem; color: #28a745;
        }
        .file-resi-badge.has-file { display: flex; }
        .btn-clear-file {
        ...
        #file-lightbox.active { display: flex; }
        #file-lightbox img {
        ...
        #resi-count-badge {
            font-size: .78rem; background: #4e73df; color: #fff;
...
#loading-log .log-ok    { color: #28a745; }
#loading-log .log-skip  { color: #ffc107; }
#loading-log .log-err   { color: #dc3545; }
#loading-log .log-info  { color: #6c757d; }
#loading-log .log-active{ color: #4e73df; font-weight: 600; }
/* MODE SELECTOR CARDS */
.mode-option { cursor: pointer; flex: 1; max-width: 200px; }
.mode-card {
    display: flex; align-items: center; justify-content: center;
    padding: 10px 16px; border-radius: 8px; border: 2px solid #dee2e6;
    background: #f8fafc; font-weight: 600; font-size: .88rem;
    transition: border-color .15s, background .15s, box-shadow .15s;
    user-select: none; cursor: pointer;
}
.mode-card .mode-check { display: none; color: #28a745; }
.mode-card.active .mode-check { display: inline; }
.mode-card--shopee.active {
    border-color: #ee4d2d; background: #fff5f3;
    box-shadow: 0 0 0 3px rgba(238,77,45,.12);
}
.mode-card--tiktok.active {
    border-color: #2f80ed; background: #f0f6ff;
    box-shadow: 0 0 0 3px rgba(47,128,237,.12);
}
.mode-card:hover { border-color: #adb5bd; background: #fff; }
</style>
...
<div class="container-fluid flex-grow-1 container-p-y">
            ...
            <div class="card-body p-3">
                @foreach ($storeIssues as $issue)
                    <div class="info-item {{ $issue['type'] === 'duplicate_resi' ? 'info-warn' : 'info-error' }}">
                        <i class="feather {{ $issue['type'] === 'duplicate_resi' ? 'icon-skip-forward text-warning' : 'icon-x-circle text-danger' }}"></i>
                        <div>
    ...
    <div class="card col-lg-12 mb-4">
        ...
        <div class="card-body">
            ...
            <div class="mt-3 w-full">
                ...
                <div class="d-flex gap-2" style="gap:10px;">
                    <label class="mode-option " id="mode_label_shopee">
                        <input type="radio" name="import_mode" id="import_mode_shopee"
                               value="shopee" checked hidden>
                    ...
                    <label class="mode-option " id="mode_label_tiktok">
                        <input type="radio" name="import_mode" id="import_mode_tiktok"
                               value="tiktok" hidden>
            ...
            <button type="button" id="btn_import_multiple"
                    class="btn btn-primary w-100 mt-3" style="font-size:1rem;padding:12px;">
                <i class="feather icon-zap mr-2"></i>
                IMPORT MULTIPLE RESI -
                <span id="import_mode_badge" class=""
                      style="">Shopee</span>
            </button>
    ...
    <div class="card col-lg-12 mb-4">
        ...
        <div class="card-body">
            ...
            <form action="{{ route('penjualan.store.multiple') }}" method="POST" id="form_multiple_resi"
                ...
                <button type="button" id="btn_add_resi_bottom"
                        class="btn-add-resi mt-3" style="display:none;">
...
// HELPERS
// ================================================================
function uid()           { return 'resi_' + (++resiIndex); }
function formatRupiah(n) { return 'Rp ' + parseInt(n).toLocaleString('id-ID'); }
function isDraftModeFor(id) { return $(`#draft_${id}`).val() === 'yes'; }
...
function checkDuplicateResi(resiVal, pesananVal, excludeId = null) {
    for (const r of resiList) {
        ...
        if (resiVal && existingResi && resiVal === existingResi) {
            return { isDuplicate: true, type: 'resi', value: resiVal, existingCard: r.uid };
        }
        if (pesananVal && existingPesanan && pesananVal === existingPesanan) {
            return { isDuplicate: true, type: 'pesanan', value: pesananVal, existingCard: r.uid };
        }
    }
    return { isDuplicate: false, type: null, value: '', existingCard: null };
}
...
function highlightExistingCard(cardId) {
    ...
    card.css({ outline: '3px solid #dc3545', borderRadius: '10px' });
...
function storeBase64ForCard(cardId, base64String, filename) {
    ...
    const raw = base64String.startsWith('data:')
        ...
        : base64String;
    resiImageMap[cardId] = { raw, filename: filename || 'resi.jpg' };
...
function renderInfoPanel(warnings, extras, stats) {
    ...
    extras.forEach(msg => {
        // Bedakan warning duplicate (icon kuning) vs error lainnya
...
function addResiCard(prefillResi = '', prefillPesanan = '', skipDupCheck = false) {
    ...
    resiList.push({ uid: id, resi: prefillResi, pesanan: prefillPesanan, items: {} });
...
function initFileResiListener(id) {
    $(`#file_resi_${id}`).on('change', function() {
        ...
        if (!file) { showFilePreview(id, null, ''); clearFileInput(id); return; }
...
function initProductSelect(id) {
    $(`#pselect_${id}`).select2({
        ...
        multiple: true, width: '100%', closeOnSelect: false,
        ajax: {
            url: "/api/product/search", dataType: "json", delay: 150, cache: true,
            data: params => ({ q: params.term, page: params.page || 1 }),
            processResults: data => ({
                results: data.map(p => ({
                    id: p.id,
                    text: "#" + p.sku + " - " + p.nama_barang + " | Stok: " + p.stok.jumlah_stok + " | " + formatRupiah(p.harga_2),
                    product: p
                }))
            })
        }
...
function initBarcodeListeners(id) {
    ...
    $(`#barcode_${id}`).on('input', function() {
        ...
        timer10 = setTimeout(() => {
            const sku = $(this).val().trim(); if (!sku) return;
...
function getResiState(id) { return resiList.find(r => r.uid === id); }
...
function addItemToCard(cardId, product, qty) {
    ...
    if (state.items[product.id]) {
        ...
        if (!draft && newQty > state.items[product.id].stok) {
        ...
        const ok = updateStockUsage(cardId, product.id, newQty, product.stok.jumlah_stok);
        if (!ok) Toast.fire({ icon: 'warning', title: `Stok konflik lintas resi!`,
            html: `SKU <b>#${product.sku}</b> — total: <b>${getTotalUsed(product.id)}</b>, stok: <b>${product.stok.jumlah_stok}</b>` });
        return;
    }
    ...
    state.items[product.id] = {
        ...
        stok: product.stok.jumlah_stok, harga_2: product.harga_2, qty, nomor_urut
    };
    const ok = updateStockUsage(cardId, product.id, qty, product.stok.jumlah_stok);
    if (!ok) Toast.fire({ icon: 'warning', title: `Stok konflik lintas resi!`,
        html: `SKU <b>#${product.sku}</b> — total: <b>${getTotalUsed(product.id)}</b>, stok: <b>${product.stok.jumlah_stok}</b>` });
...
function calcSubtotal(cardId) {
    const state = getResiState(cardId); if (!state) return;
    let sub = 0;
...
function validateStockFor(product, isDraft) {
    ...
    if (!isDraft) {
        if (stok <= 0)  { Toast.fire({ icon: 'error',   title: `SKU #${product.sku} Stok habis!` });           return false; }
        if (stok < min) { Toast.fire({ icon: 'error',   title: `SKU #${product.sku} Stok dibawah minimum!` }); return false; }
    } else {
        if (stok <= 0) Toast.fire({ icon: 'warning', title: `SKU #${product.sku} Stok habis, dicatat draft.` });
    }
    if (stok <= 5  && stok > 0) Toast.fire({ icon: 'warning', title: `SKU #${product.sku} Stok kritis (${stok}).` });
    else if (stok <= 10 && stok > 0) Toast.fire({ icon: 'warning', title: `SKU #${product.sku} Stok menipis (${stok}).` });
    return true;
}
...
$(document).on('change', '.qty', function() {
    ...
    const draft     = isDraftModeFor(cardId);
    if (!val || val <= 0) {
        $(this).val(1); item.qty = 1;
        Toast.fire({ icon: 'error', title: 'Qty tidak boleh 0 atau minus!' });
        updateRowCard(cardId, productId); return;
    }
    if (!draft && val > item.stok) {
        Toast.fire({ icon: 'error', title: `Qty melebihi stok (${item.stok})` });
        $(this).val(item.qty); return;
    }
    if (draft && val > item.stok)
        Toast.fire({ icon: 'warning', title: `Qty ${val} melebihi stok (${item.stok}), dicatat draft.` });
    item.qty = val;
...
$('#btn_add_resi_manual, #btn_add_resi_bottom').on('click', function() {
    addResiCard();
    $('html, body').animate({ scrollTop: $('.resi-card').last().offset().top - 80 }, 300);
});

$('#btn_apply_ds_all').on('click', function() {
    const dsId = $('#set_all_dropshipper').val();
    if (!dsId) { Toast.fire({ icon: 'warning', title: 'Pilih dropshipper dulu!' }); return; }
...
$('#form_multiple_resi').on('submit', function(e) {
    e.preventDefault();
    if (resiList.length === 0) {
        Swal.fire({ icon: 'warning', title: 'Tambahkan minimal 1 resi!' }); return;
    }
    ...
    Object.entries(stockUsageMap).forEach(([productId, entry]) => {
        ...
        if (totalUsed > entry.stok) {
            valid = false;
            errors.push(`Konflik stok lintas resi: total qty <b>${totalUsed}</b> melebihi stok <b>${entry.stok}</b>`);
        }
    });
    ...
    resiList.forEach((r, idx) => {
        ...
        if (Object.keys(r.items).length === 0) {
            valid = false; errors.push(`Resi #${idx + 1}: belum ada barang.`); return;
        }
        Object.values(r.items).forEach(item => {
            if (!draft && item.qty > item.stok) {
                valid = false;
                errors.push(`Resi #${idx + 1} SKU #${item.sku}: qty ${item.qty} melebihi stok ${item.stok}`);
            }
        });
    });
