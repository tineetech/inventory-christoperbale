                                                    ...
                                                    <tbody>
                                                        @forelse ($barang as $index => $sup)
                                                            <tr data-id="{{ $sup->id }}">
                                                                ...
                                                                @if (Auth::guard('pengguna')->user()->role->nama_role === 'super_admin')
                                                                    <td>Rp {{ number_format($sup->harga_1, 0, ',', '.') }}</td>
                                                                @endif
...
function updateBulkBar() {
    // Kumpulkan dari table (desktop) & card (mobile)
...
async function bulkDelete() {
    ...
    try {
        ...
        if (json.success) {
            Swal.fire({ icon: 'success', title: 'Berhasil', text: json.message, timer: 1500 })
...
function showImportModal() {
    Swal.fire({
        ...
        if (result.isConfirmed && result.value) {
            const { imported, skipped, details } = result.value;
