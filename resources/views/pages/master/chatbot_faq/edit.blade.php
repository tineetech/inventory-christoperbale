    ...
    <div class="layout-content">
        <div class="container-fluid flex-grow-1 container-p-y">
            ...
            <div class="card col-lg-12 mb-4">
                <h6 class="card-header">Edit FAQ Chatbot</h6>
                <div class="card-body">
                    <form action="{{ route('chatbot_faq.update', $faq->id) }}" method="POST" id="faqForm">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="pertanyaan" class="form-control @error('pertanyaan') is-invalid @enderror"
                                placeholder="Contoh: Bagaimana cara melakukan refund?" value="{{ old('pertanyaan', $faq->pertanyaan) }}" required>
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="jawaban" class="form-control @error('jawaban') is-invalid @enderror" rows="5"
                                placeholder="Jawaban lengkap dari pertanyaan FAQ..." required>{{ old('jawaban', $faq->jawaban) }}</textarea>
                        ...
                        <div class="form-row">
                            <div class="form-group col-md-6">
                                ...
                                <input type="text" name="keywords" class="form-control @error('keywords') is-invalid @enderror"
                                    placeholder="Contoh: refund, uang kembali, pengembalian"
                                    value="{{ old('keywords', is_array($faq->keywords) ? implode(', ', $faq->keywords) : $faq->keywords) }}">
                                @error('keywords')<div class="invalid-feedback">{{ $message }}</div>@enderror
                            </div>
                            <div class="form-group col-md-3">
                                <label class="form-label">Urutan</label>
                                <input type="number" name="urutan" class="form-control @error('urutan') is-invalid @enderror"
                                    placeholder="0" value="{{ old('urutan', $faq->urutan) }}" min="0">
                                @error('urutan')<div class="invalid-feedback">{{ $message }}</div>@enderror
                            </div>
                            <div class="form-group col-md-3">
                                <label class="form-label">Status</label>
                                <select name="is_active" class="form-control @error('is_active') is-invalid @enderror">
                                    <option value="1" {{ old('is_active', $faq->is_active ? '1' : '0') == '1' ? 'selected' : '' }}>Aktif</option>
                                    <option value="0" {{ old('is_active', $faq->is_active ? '1' : '0') == '0' ? 'selected' : '' }}>Nonaktif</option>
                                </select>
                                ...
                                <div class="form-row align-items-center">
                                    <div class="form-group col-md-6 mb-0">
                                        <div class="custom-control custom-checkbox">
                                            <input type="checkbox" class="custom-control-input" id="quick_question"
                                                name="quick_question" value="1" {{ old('quick_question', $faq->quick_question ? '1' : '') ? 'checked' : '' }}>
                                    ...
                                    <div class="form-group col-md-6 mb-0">
                                        ...
                                        <input type="number" name="quick_question_order" class="form-control @error('quick_question_order') is-invalid @enderror"
                                            placeholder="0" value="{{ old('quick_question_order', $faq->quick_question_order) }}" min="0">
