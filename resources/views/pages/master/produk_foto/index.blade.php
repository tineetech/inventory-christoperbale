...
@section('content')
    <div class="layout-content">
        <div class="container-fluid flex-grow-1 container-p-y">
            <h4 class="font-weight-bold py-3 mb-0">Produk Foto</h4>
            <div class="text-muted small mt-0 mb-4 d-block breadcrumb">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="#"><i class="feather icon-home"></i></a></li>
                    <li class="breadcrumb-item"><a href="#">Barang - Web</a></li>
                    <li class="breadcrumb-item active">Produk Foto</li>
                </ol>
            </div>
                ...
                <div class="card-header" style="border: none !important">
                    <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-2">
                        <h6 class="card-header-title mb-0">
                            <i class="feather icon-image mr-2"></i> Data Produk Foto
                        </h6>
                    </div>

                    <form method="GET" action="{{ route('produk_foto.index') }}" id="filterForm">
                        <div class="d-flex flex-wrap align-items-center gap-2">
                            <div class="flex-grow-1" style="min-width: 160px; max-width: 100%;">
                                <div class="input-group input-group-sm">
                                    ...
                                    <input type="text" name="search" class="form-control form-control-sm"
                                        placeholder="Cari foto..." value="{{ request('search') }}">
                                </div>
                            </div>
                            ...
                            @if (request('search'))
                                <a href="{{ route('produk_foto.index', ['per_page' => request('per_page', 10)]) }}"
                        ...
                        <div class="tab-pane fade show active pb-3" style="width: 95%">
                            ...
                            <div class="d-none d-md-block" style="overflow-x: auto">
                                <table class="table table-modern table-hover mb-0" id="table">
                                    ...
                                    <tbody>
                                        @forelse ($foto as $index => $f)
                                            <tr>
                                                <td>{{ $foto->firstItem() + $index }}</td>
                                                <td>{{ $f->produk->nama_produk ?? '-' }}</td>
                                                <td>
                                                    <img src="{{ asset('storage/' . $f->foto) }}" alt="foto"
                                        ...
                                        @empty
                                            <tr>
                                                <td colspan="6" class="text-center text-muted py-5">
                                                    ...
                                                    <p class="mt-2 mb-0">Tidak ada data foto.</p>
                                                </td>
                                            </tr>
                                        @endforelse
                                    </tbody>
                            ...
                            <div class="d-flex flex-wrap justify-content-between align-items-center px-3 py-3 border-top gap-2">
                                <div class="text-muted small">
                                    @if ($foto->total() > 0)
                                        Showing <strong>{{ $foto->firstItem() }}</strong>
                                        to <strong>{{ $foto->lastItem() }}</strong>
                                        of <strong>{{ $foto->total() }}</strong> entries
                                        @if (request('search'))
                                ...
                                @if ($foto->hasPages())
                                    <ul class="pagination pagination-sm mb-0 flex-wrap">
                                        <li class="page-item {{ $foto->onFirstPage() ? 'disabled' : '' }}">
                                            <a class="page-link"
                                                href="{{ $foto->previousPageUrl() . '&per_page=' . request('per_page', 10) . (request('search') ? '&search=' . request('search') : '') ?? '#' }}">
                                                <i class="feather icon-chevron-left"></i>
                                            </a>
                                        </li>

                                        @php
                                            $currentPage = $foto->currentPage();
                                            $lastPage = $foto->lastPage();
                                            if ($lastPage <= 7) {
                                                $start = 1;
                                                $end = $lastPage;
                                        ...
                                        @if ($lastPage > 7)
                                            <li class="page-item {{ $currentPage == 1 ? 'active' : '' }}">
                                                <a class="page-link"
                                                    href="{{ $foto->url(1) }}&per_page={{ request('per_page', 10) }}{{ request('search') ? '&search=' . request('search') : '' }}">
                                                    1
                                                </a>
                                            </li>
                                            ...
                                            @for ($i = $start; $i <= $end; $i++)
                                                <li class="page-item {{ $currentPage == $i ? 'active' : '' }}">
                                                    <a class="page-link"
                                                        href="{{ $foto->url($i) }}&per_page={{ request('per_page', 10) }}{{ request('search') ? '&search=' . request('search') : '' }}">
                                                        {{ $i }}
                                                    </a>
                                                </li>
                                            @endfor
                                            ...
                                            <li class="page-item {{ $currentPage == $lastPage ? 'active' : '' }}">
                                                <a class="page-link"
                                                    href="{{ $foto->url($lastPage) }}&per_page={{ request('per_page', 10) }}{{ request('search') ? '&search=' . request('search') : '' }}">
                                                    {{ $lastPage }}
                                                </a>
                                            </li>
                                        @else
                                        ...
                                        <li class="page-item {{ !$foto->hasMorePages() ? 'disabled' : '' }}">
                                            <a class="page-link"
                                                href="{{ $foto->nextPageUrl() . '&per_page=' . request('per_page', 10) . (request('search') ? '&search=' . request('search') : '') ?? '#' }}">
